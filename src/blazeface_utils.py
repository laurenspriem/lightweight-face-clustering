...
class Anchor:
    ...
    def to_string(self):
        return "x_center: {:}, y_center: {:}, h: {:}, w: {:}".format(
            self.x_center, self.y_center, self.h, self.w
        )
...
def gen_anchors(options):
    ...
    while layer_id < options.strides_size:
        ...
        while (
            ...
            if int(options.strides_size) == 1:
                scale = options.min_scale + (
                    options.max_scale - options.min_scale
                ) * 1.0 * last_same_stride_layer / (
                    options.strides_size
                )  # NOTE: removed -1.0 in the last bracket for use of MediaPipe model which has only 1 layer
            else:
                scale = options.min_scale + (
                    options.max_scale - options.min_scale
                ) * 1.0 * last_same_stride_layer / (options.strides_size - 1)
            if last_same_stride_layer == 0 and options.reduce_boxes_in_lowest_layer:
                # For first layer, it can be specified to use predefined anchors. We currently don't use this in the MediaPipe model.
            ...
            last_same_stride_layer += 1
        for i, _ in enumerate(aspect_ratios):
